...
export default (options: ConditionCardOptions | MockConditionCardOptions): boolean => {
  ...
  if (first < 0 || first > 31 || second < 0 || second > 31) {
    app.logError("daynum_between_daynum: Argument 'dayOne' and/or 'dayTwo' invalid...");
  ...
  if (today >= first && today <= second) {
    app.log(
      `daynum_between_daynum: Today(${today}) is (>= to first(${first}) && <= to second(${second})). Inside for this month!`
    );
    return true;
  }
  ...
  if (second < first && today >= first && today >= second) {
    app.log(
      `daynum_between_daynum: Second(${second}) is < first(${first}) && today(${today}) is (>= to first(${first}) && >= to second(${second})). Inside for this month!`
    );
    return true;
  }
  ...
  if (second < first && today >= first && today <= second) {
    app.log(
      `daynum_between_daynum: Second(${second}) is < first(${first}) && today(${today}) is (>= to first(${first}) && <= to second(${second})). Inside for next month!`
    );
    return true;
  }
